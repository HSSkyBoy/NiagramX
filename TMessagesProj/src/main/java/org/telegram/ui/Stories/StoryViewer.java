                    ...
                    if (inSwipeToDissmissMode) {
                        if (allowSwipeToReply) {
                            ...
                            if (swipeToReplyOffset > maxOffset && !swipeToReplyWaitingKeyboard) {
                                ...
                                showKeyboard();
                                try {
                                    if (!NyaConfig.disableVibration.Bool()) windowView.performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP);
                            ...
                            if (peerView != null && peerView.currentStory != null && peerView.currentStory.uploadingStory == null && peerView.currentStory.isVideo()) {
                                ...
                                if (videoDuration > 0) {
                                    ...
                                    if ((int) (nowSeek * 10) != (int) (wasSeek * 10)) {
                                        try {
                                            if (!NyaConfig.disableVibration.Bool()) peerView.performHapticFeedback(9, HapticFeedbackConstants.FLAG_IGNORE_VIEW_SETTING);
